...
export default function TrendDetail({ userdata }){
    ...
    const fetchtag = () => {
        fetch(`http://localhost:8000/api/posts_by_hashtag/${tag1.name}/`, {
            method: 'GET',
            credentials: 'include',
        })
        .then(response => {
    ...
    return (
        <>
        <Helmet>
            <title>{`#${tag1.name} - Search / X`}</title>
        </Helmet>
        <div className="main">
            <div className="inner">
                    ...
                    <div className="centerinner">
                        <div className="centertop">
                            <div className="centersearch">
                                <form action="#">
                                    <input type="text" placeholder="Search" value={`#${tag1.name}`} readOnly></input>
                                </form>
                            </div>
                                                ...
                                                <div className="post" key={post.id}>
                                                    ...
                                                    <div className="postright">
                                                        ...
                                                        <div className="postcontent">
                                                                {post.tags.map(tag => (
                                                                    <NavLink to={`/trend/${tag.name}`} style={{color: 'dodgerblue'}} key={tag.id} onClick={handleRefresh}>
                                                                        {tag1.name === tag.name ? (
                                                                            <b>#{tag.name}</b>
                                                                        ):(<>
                                                                            #{tag.name}
